...
# 6. Seed demo data (optional, but highly recommended for demo)
log_info "Seeding demo users and organization..."

# Create an organisation first
ORG_ID="aaaaaaaa-0000-0000-0000-000000000001"
TENANT_ID="bbbbbbbb-0000-0000-0000-000000000001"
log_info "Creating demo organization 'Acme Airlines' (ID: ${ORG_ID})..."
docker exec -it $(docker compose ps -q postgres) psql -U postgres -d fsbs -c "
...
" || log_warn "Failed to create demo organization (might already exist)."
log_info "Demo organization setup complete."

# Seed common users
log_info "Seeding common demo users..."
DEMO_ROLES=("SystemAdmin" "ScheduleAdmin" "CourseDirector" "Instructor" "Management" "SalesStaff" "InternalStudent" "PrivateCustomer")
for ROLE in "${DEMO_ROLES[@]}"; do
  EMAIL="${ROLE,,}@fsbs.local"
  log_info "Seeding user: ${EMAIL} with role: ${ROLE}..."
  if [[ "$ROLE" == "CorporateManager" || "$ROLE" == "CorporateStudent" ]]; then
    # Corporate roles need an orgId in the seed request
    curl -s -X POST "${API_URL}/dev/users/seed?email=${EMAIL}&role=${ROLE}&orgId=${ORG_ID}" -k > /dev/null || log_warn "Failed to seed ${EMAIL}."
  else
    curl -s -X POST "${API_URL}/dev/users/seed?email=${EMAIL}&role=${ROLE}" -k > /dev/null || log_warn "Failed to seed ${EMAIL}."
  fi
done
log_info "Demo users seeded. You can get their tokens via the /dev/auth/token endpoint."
log_info "Example: curl -s -X POST \"${API_URL}/dev/auth/token?email=systemadmin@fsbs.local&role=SystemAdmin\" -k | jq ."
